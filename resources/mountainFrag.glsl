...
vec3 render( vec3 ro, vec3 rd ) {
	...
	vec3 color = vec3(0.7,0.6,0.6); // base color is sky color
	...
	if (res.y>-0.5 && res.y<0.5) {
		...
		color = mix( vec3(0.7, 0.1, 0.0), vec3(0.8, 0.6, 0.3), step(0.9, nor.y) );
		...
		color = mix( vec3(0.8, 0.5, 0.4), color, n*smoothstep(0.0, 0.7, nor.y) ); 
	...
	if (res.y>0.5) {
		...
		color = mix( vec3(0.7, 0.4, 0.0), vec3(0.7, 0.4, 0.3), nor.y );
	...
	// fog
	float fog = exp(-0.05 * res.x*res.x); // exponentioal fog equation
